...
class GameOfLife {
  ...
  newState() {
    for (var i = 0; i <= this.rows * this.columns - 1; i++) {
      var activeNeighbours = this._gridItems[i].neighbours.filter(
        (el) => this._gridItems[el] && this._gridItems[el].isAlive
      );
      var numberOfActiveNeighbours = activeNeighbours.length;
      if (
        this._gridItems[i].isAlive &&
        (numberOfActiveNeighbours === 2 || numberOfActiveNeighbours === 3)
      )
        this._gridItems[i].newState = true;
      else if (!this._gridItems[i].isAlive && numberOfActiveNeighbours === 3)
        this._gridItems[i].newState = true;
      else {
        this._gridItems[i].newState = false;
      }
    }
  ...
  generateRandomPattern() {
    this._gridItems.forEach((el) =>
      Math.random() > 0.5 ? el.live() : el.dead()
    );
  }
