class Cell {
  ...
  calcElements(row, offset, numberOfElementsPerRow) {
    var result = [];
    if (offset - 1 >= 0 && offset - 1 <= numberOfElementsPerRow - 1)
      result.push(this.calcIndex(row, offset - 1, numberOfElementsPerRow));
    if (offset >= 0 && offset <= numberOfElementsPerRow - 1)
      result.push(this.calcIndex(row, offset, numberOfElementsPerRow));
    if (offset + 1 >= 0 && offset + 1 <= numberOfElementsPerRow - 1)
      result.push(this.calcIndex(row, offset + 1, numberOfElementsPerRow));
    return result;
  }
  ...
  calcNeighbours(numOfRows, numberOfElements) {
    ...
    if (row - 1 >= 0)
      neighbours = neighbours.concat(
        this.calcElements(row - 1, offset, numberOfElementsPerRow)
      );
    if (row >= 0)
      neighbours = neighbours.concat(
        this.calcElements(row, offset, numberOfElementsPerRow)
      );
    if (row + 1 >= 0)
      neighbours = neighbours.concat(
        this.calcElements(row + 1, offset, numberOfElementsPerRow)
